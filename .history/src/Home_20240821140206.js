...
function Home({ user }) {
    ...
    const [isSearchPopupOpen, setIsSearchPopupOpen] = useState(false); // Add state for search popup
    ...
    useEffect(() => {
        if (searchTerm) {
            ...
            setIsSearchPopupOpen(true); // Open search popup when there are results
        } else {
            setSearchResults([]);
            setIsSearchPopupOpen(false); // Close search popup when search term is cleared
        }
                ...
                <div className="container">
                    ...
                    <div className="main-container">
                        {sortedListings.map((listing) => (
                            <div key={listing.id} className="itemlisting-">
                                <h2>{listing.name}</h2>
